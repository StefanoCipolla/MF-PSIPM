#!/usr/bin/env python3
"""PSIPM-V3: generic homogeneous self-dual proximal IPM.
...
class TreePreconditioner:
    ...
    def __init__(self, topology: NetworkTopology, active_columns: np.ndarray):
    ...
    def record_iterations(self, iterations: int) -> None:
        del iterations
...
class StableColumnPreconditioner:
    ...
    def __init__(self, A: sp.csc_matrix, options: V3Options):
        self.diagonal = DiagonalPreconditioner(A)
        self.impl = None
        self._backend = "diagonal"
        self._required = options.generic_preconditioner == "stable"
        self._retry_growth = 1.35
        ...
        try:
            ...
            self.impl = StableActiveColumnPreconditioner(
                ...
                refresh_cg=options.generic_refresh_cg,
    ...
    def record_iterations(self, iterations: int) -> None:
        if self.impl is not None:
            ...
            if (
                iterations >= self.impl.refresh_cg
                and self.impl.core_target < self.impl.max_core_target
            ):
                grown = max(
                ...
                # Force stable retention plus leverage-based replenishment at
                # the next Newton matrix, where the current D is available.
...
def _make_preconditioner(
    ...
    if structure.is_network:
        return TreePreconditioner(structure, columns)
    return StableColumnPreconditioner(A[:, columns].tocsc(), options)
...
def _full_metrics(lp: CanonicalLP, columns, state: HSDState, reduced=None):
    ...
    primal_residual = lp.A[:, columns] @ x_active - lp.b
    if reduced is None:
...
def solve(lp: CanonicalLP, options: Optional[V3Options] = None) -> V3Result:
    ...
    preconditioner = _make_preconditioner(structure, lp.A, columns, options)
    drop_age = np.zeros(lp.c.size, dtype=np.uint8)
    setup_seconds = time.perf_counter() - setup_started
    if options.print_level:
        print(
            f"PSIPM-V3 HSD | {lp.name or '<memory>'} | m {lp.A.shape[0]} "
    ...
    while iterations < options.max_iterations:
        ...
        preconditioner = _make_preconditioner(
            structure, lp.A, columns, options
        )
...
def _parser():
    ...
    parser.add_argument("--generic-refresh-cg", type=int, default=30)
